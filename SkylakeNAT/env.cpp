...
std::string CPUID() {
	...
	sprintf(szBuf, "%08X", a1);
	xcpuid = szBuf;

	sprintf(szBuf, "%08X", a2);
	xcpuid += szBuf;
...
void ComputeMD5(const std::string& s, const unsigned char* md5, int& md5len) {
	if (md5len <= 0 || NULL == md5)
		return;
	md5len = md5len > sizeof(MD5::HEX) ? sizeof(MD5::HEX) : md5len;
...
std::string GetVolumelabelNumber(const std::string& volumelabel) {
	...
	char szBuf[100];
	sprintf(szBuf, "%08X", serialNumber);
	return szBuf;
}

std::string CreateGuid() {
	return Guid::NewGuid().ToString(Guid::TOSTRINGFORMAT_B);
}
...
std::string& GetApplicationKey() {
	static std::string application_key = "";

	if (application_key.empty()) {
...
#pragma warning(once: 4996)
		...
		char sz[1000];
		application_key = "CPUID: " + CPUID();
		application_key += "\r\n";
		application_key += "VOLUMEID: " + GetVolumelabelNumber(GetPreferredVolumelabel());
		application_key += "\r\n";
		application_key += "COMPONENTID: " + Tap::GetDefaultComponentId();
		application_key += "\r\n";
		application_key += "CPUNUM: " + std::string(itoa(GetProcessorCount(), sz, 10));
		application_key += "\r\n";
		application_key += "OSVERISON: " 
			+ std::string(ultoa(osver.dwMajorVersion, sz, 10)) 
			+ "." 
			+ std::string(ultoa(osver.dwMinorVersion, sz, 10));
		application_key += "\r\n";

		std::shared_ptr<Tap::NetworkInterface> ni = Tap::GetPreferredNetworkInterface();
		if (ni) {
			application_key += "MAC: " + ni->MacAddress;
			application_key += "\r\n";
			application_key += "IFID: " + ni->Id;
			application_key += "\r\n"; 
			application_key += "IFINDEX: " + std::string(ultoa(ni->IfIndex, sz, 10));
		}

		application_key = ComputeMD5(application_key);
	}
	return application_key;
}

int GetApplicationId() {
	static int applicationId = GetHashCode(ComputeMD5(GetApplicationKey()));
	return applicationId;
}

int GetHashCode(const std::string& s) {
	return GetHashCode(s.data(), (int)s.size());
}
...
int GetHashCode(const void* data, int datalen) {
	...
	unsigned int hash = crc ^ 0xFFFFFFFF;
	return (int)hash;
}
...
std::string GetCurrentTimeCrt(const char* fmt) {
	SYSTEMTIME tm{ 0 };
	GetLocalTime(&tm);
...
std::string GetExecutionFileName() {
	...
	DWORD dw = GetModuleFileNameA(NULL, szName, MAX_PATH);
	if (!dw)
		return "";
	else {
		std::string r = std::string(szName, dw);
		size_t pos = r.rfind('\\');
...
std::string GetFullExecutionFilePath() {
	...
	DWORD dw = GetModuleFileNameA(NULL, szFileName, sizeof(szFileName));
	if (!dw)
		return "";
	return szFileName;
}
...
std::string ToString(int value, int radix) {
	char sz[255];
	itoa(value, sz, radix);
	return sz;
}
...
bool WriteDumpFile(const char* dumpFilePathName, EXCEPTION_POINTERS* exception) {
	if (NULL == dumpFilePathName || NULL == exception) {
		return false;
	}

	// 创建Dump文件
	//
	HANDLE hFile = CreateFileA(dumpFilePathName, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	// Dump信息
	//
	MINIDUMP_EXCEPTION_INFORMATION di{ 0 };
	...
	// 写入Dump文件内容
	//
	bool success = FALSE != MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), hFile, MiniDumpNormal, &di, NULL, NULL);
